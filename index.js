...
function App() {
      ...
      .then(function (response) {
        ...
        var jobHTMLBlock = function jobHTMLBlock(job) {
          return '\n          <a href="'
            .concat(
              job.absolute_url,
              '" target="_blank" class="careers-item">\n            <h2 class="careers-item-headline">'
            )
            .concat(
              job.title,
            ...
            .concat(
              job.location.name.replace(", United States", ""),
              '\n            </p>\n\n            <div class="careers-item-cta button -cta -color-dark" href="'
            )
            .concat(
              job.absolute_url,
        ...
        var departmentHTMLBlock = function departmentHTMLBlock(department) {
          return '\n          <div class="careers-department">\n            <h3 class="careers-deparment-headline _top-border-headline">'
            .concat(department.name, "</h3>\n\n            ")
            .concat(
              department.jobs.reduce(function (listing, job) {
        ...
        jobsByDepartment.forEach(function (department) {
          careersListEl.insertAdjacentHTML(
            "beforeend",
            departmentHTMLBlock(department)
          );
        });
      });
  };
    ...
    items.forEach(function (item) {
      item.addEventListener("click", function () {
        if (this.classList.contains(active)) {
          ...
          var currentlySelected = document.querySelector(
  ...
  var videoPopup = function videoPopup() {
    ...
    document
      .querySelector(".popupVideo-close")
      ...
      triggers.forEach(function (trigger) {
        trigger.addEventListener("mouseover", function (event) {
          ...
          var highlight = document.querySelectorAll(
            'svg [data-name="'.concat(target, '"] image')
          );
    ...
    var clickInteraction = function clickInteraction() {
      triggers.forEach(function (trigger) {
        trigger.addEventListener("click", function (event) {
          var targetRegionName = event.target.closest(element).dataset
            .brainHover;
          var targetRegionImage = document.querySelector(
            'svg [data-name="'.concat(targetRegionName, '"] image')
          );
          var currentlySelectedRegion = document.querySelector(
            ".applicationsTwo-brains image.-active"
          );
          if (targetRegionImage.classList.contains("-active")) {
            targetRegionImage.classList.remove("-active");
          } else {
            currentlySelectedRegion &&
              currentlySelectedRegion.classList.remove("-active");
